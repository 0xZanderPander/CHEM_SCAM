...
export async function GET(_request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const [duplicates, comments] = await Promise.all([
    query<DuplicateReport>(
      "SELECT id, report_id, nickname, description, website, created_at FROM duplicate_reports WHERE report_id = $1 AND publication_state = 'published' AND removed_at IS NULL ORDER BY created_at DESC",
      [id],
    ),
    query<Comment>(
      "SELECT id, report_id, nickname, comment, created_at, flag_count, publication_state FROM comments WHERE report_id = $1 AND publication_state = 'published' AND removed_at IS NULL ORDER BY created_at ASC",
      [id],
    ),
  ]);
