...
export function rateLimitLockKey(name: string, requester: string, scopeKey: string) {
  return `rate-limit:${name}:${requester}:${scopeKey}`;
}
...
export async function enforceRateLimit(request: NextRequest, name: LimitName, scopeKey = "global", now = new Date()) {
  ...
  const configured = Number(process.env[setting.env] || setting.limit);
  const limit = Number.isInteger(configured) && configured > 0 ? configured : setting.limit;
  ...
  await transaction(async (client) => {
    ...
    // The transaction-scoped advisory lock serializes count-and-insert for the
    // exact privacy-preserving bucket without adding another service or table.
    await client.query("SELECT pg_advisory_xact_lock(hashtextextended($1, 0))", [rateLimitLockKey(name, requester, scopeKey)]);
